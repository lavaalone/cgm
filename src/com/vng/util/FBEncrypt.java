...
public class FBEncrypt
{
	...
	private int						_max_input_len = 1024*10;
	...
	public FBEncrypt(byte[] bin, boolean reuse_bin)
	{
		...
		if (reuse_bin)
		{
			...
			{
				LogHelper.Log("updateByteArrayData failed!"); 
			}
		}
	}
	...
	public boolean decode(byte[] bin)
	{
		...
		{
			com.vng.log.LogHelper.LogException("FBDecode", ex);
	...
	public void displayDataPackage()
	{
		if (set.size() != 0)
		{
			if (toByteArray() == null)
			{
				LogHelper.Log("Byte array is null"); 
			}
		...
		else if (toByteArray() != null)
		{
			if (set.size() == 0)
			{
				LogHelper.Log("Hashmap is empty"); 
			}
	...
	public void display()
	{
		...
		while (i.hasNext())
		{
			...
			LogHelper.Log(me.getKey() + ": " + me.getValue());
		}	
	}
	...
	public void updateValue(HashMap<String, Object> _set_new)
	{
		...
		while (i.hasNext())
		{
			...
			{
				LogHelper.Log("Update " + key + " failed!"); 
			}
		}
	...
	public void updateValue(String _key, Object _val, boolean _force_add)
	{
		...
		{
			LogHelper.Log("Update " + _key + " failed!"); 
			return;
		}
...
//				addFloat((String)me.getKey(), ((Float)val).floatValue());
			...
			{
				LogHelper.Log("updateByteArrayData.. unknown type.");
				return false;
			}
		}
...
public class FBEncrypt
{
	...
	public boolean decode(byte[] bin)
	{
		...
		try
		{
			...
			{
				//read key len
				int key_len = dis.readByte()&0xFF;
			
				//read key
				byte[] bkey = new byte[key_len];
				dis.read(bkey);
				String key = new String(bkey, java.nio.charset.StandardCharsets.US_ASCII);
				...
				switch (type)
				{
					...
					case BINARY:
						int b_len = dis.readShort() & 0xffff;
					...
					case STRING_ANSI:
						int sa_len = dis.readShort() & 0xffff;
						String sa_str = "";
						if (sa_len > 0 && sa_len < _max_input_len)	//max str len < _max_input_len
						{
							byte[] data = new byte[sa_len];
					...
					case STRING_UNICODE:
						int su_len = (dis.readShort() & 0xffff) * 2;
						String su_str = "";
						if (su_len > 0 && su_len < _max_input_len)	//max str len < _max_input_len
						{
							byte[] data = new byte[su_len];
	...
	private boolean decodeArray(String key, DataInputStream dis)
	{
		try
		{
			int type = dis.readByte()&0xFF;
	...
	public boolean updateField(String _key, Object _val, boolean _force_add)
	{
		boolean updated = false;
	...
	public void updateByteArrayData()
	{
		resetByteArray();
		...
		while (i.hasNext())
		{
			...
			if (val instanceof String)
			{
				addString((String)me.getKey(), (String)val);
			}
			else if (val instanceof byte[])
			{
				addBinary((String)me.getKey(), (byte[])val);
			}
			else if (val instanceof Byte)
			{
				addByte((String)me.getKey(), ((Byte)val).byteValue());
			}
			else if (val instanceof Short)
			{
				addShort((String)me.getKey(), ((Short)val).shortValue());
			}
			else if (val instanceof Integer)
			{
				addInt((String)me.getKey(), ((Integer)val).intValue());
			}
			else if (val instanceof Long)
			{
				addLong((String)me.getKey(), ((Long)val).longValue());
			}
			else if (val instanceof Float)
			{
				addFloat((String)me.getKey(), ((Float)val).floatValue());
			...
			{
				addDouble((String)me.getKey(), ((Double)val).doubleValue());
			}
		}
	}
	...
	public void appendBinary(byte[] bin)
	{
		try
		{
			dos.write(bin, 0, bin.length);
	...
	public void addStringANSI(String key, String value)
	{
		try
		{
			//write key len
	...
	public void addStringUNICODE(String key, String value)
	{
		try
		{
			//write key len
	...
	public void addBinary(String key, byte[] value)
	{
		if (value == null)
		{
			LogHelper.Log("-------------- NULLLLLLLLLLLLLLLLL"); 
		}
	...
	public void addByte(String key, int value)
	{
		try
		{
			//write key len
	...
	public void addShort(String key, int value)
	{
		try
		{
			//write key len
	...
	public void addArray(String key, Object value) throws Exception
	{
		//write key len
		dos.writeByte(key.length());
		//write key
		...
		switch (value.getClass().getName())
		{
			...
			default:
			{
				throw new Exception("Dont support array type: " + value.getClass());
			}
		}
	}
}
