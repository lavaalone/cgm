...
public class WinValidatingReceiptHandler
{
	Client _client = null;
	String _data = null;
	...
	public WinValidatingReceiptHandler(Client client, String data)
	{
		_client = client;
		_data = data;
	}
	...
	public void Execute() // throws Exception
	{
		...
		try
		{
			...
			if (obj != null)
			{
				_product_id = (String)obj;
			}
			else
			{
				transactionresult = TRANSACTION_EXCEPTION;
				LogHelper.Log("Invalid transaction id := " + _transaction_id);
			}
		}
		catch (Exception e)
		{
		...
		if (load_result && (transactionresult == TRANSACTION_OK))
		{
			...
			for (int i = 0; i < Server.s_globalDB[DatabaseID.SHEET_PAYMENT].length; i++)
			{
				...
				String product_id = Misc.PARSE_STRING(Server.s_globalDB[DatabaseID.SHEET_PAYMENT][i][DatabaseID.PAYMENT_PACK_WIN_PRODUCT_ID]);
				...
				if (_product_id.equals(product_id))
				{
					...
					try
					{
						if (GameUtil.GetUserInfo(_user_id).getDeviceOS().equals("WINPHONE"))
						{
							String paid_count = GameUtil.GetUserMisc(_user_id).Get("paid_count");
							if (paid_count.equals("")) // first time user winphone
							{
								diamond_bonus = diamond_bonus + (int)(diamond_real * 0.5); // bonus 50%
								GameUtil.GetUserMisc(_user_id).Set("paid_count", "1");
							...
							else
							{
								...
								if (obj1 != null && obj2 != null)
								{
									...
									if (Misc.InEvent(start_bonus_lumina, end_bonus_lumina) && Integer.parseInt(paid_count) == 1)
									{
										...
										GameUtil.GetUserMisc(_user_id).Set("paid_count", "2");
