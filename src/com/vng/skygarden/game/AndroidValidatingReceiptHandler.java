...
public class AndroidValidatingReceiptHandler
{
	Client _client = null;
	String _data = null;
	...
	public AndroidValidatingReceiptHandler(Client client, String data)
	{
		_client = client;
		_data = data;
	}
	...
	public void Execute() // throws Exception
	{
		...
		try
		{
			...
			for (String s : aos)
			{
				...
				else if (s.contains("purchase_token"))
				{
					_purchase_token = s.split("=")[1];
				}
			...
			LogHelper.Log("_purchase_token := " + _purchase_token);
			LogHelper.Log("_order_id := " + _order_id);
		}
		catch (Exception e)
		{
		...
		int log_iLevel = 0;
		int log_chargeType = 5; // 5 : android, 6: apple
		String log_sUserIP = "";
		...
		{
			userbin = null;
			LogHelper.Log("AndroidValidatingReceiptHandler.. can not get user info data.");
			LogHelper.LogException("AndroidValidatingReceiptHandler.LoadUserData", e);
		}
		...
		try
		{
			Object obj = DBConnector.GetMembaseServer(_user_id).Get(_purchase_token);
			if (obj != null)
			{
				String s = (String)obj;
				JsonObject purchase_data = new JsonParser().parse(s).getAsJsonObject();
				_product_id = purchase_data.get("productId").toString().replace("\"", "");
				if (_product_id == null || _product_id.length() == 0)
				{
					transactionresult = TRANSACTION_EXCEPTION;
					LogHelper.Log("Invalid product id := " + _product_id);
				}
			}
			else
			{
				LogHelper.Log("Can not find purchase token := " + _purchase_token);
				transactionresult = TRANSACTION_EXCEPTION;
			}
		}
		catch (Exception e)
		...
		if (load_result && (transactionresult == TRANSACTION_OK))
		{
			...
			// base on product id, decides how many diamond will be added to user's money
			for (int i = 0; i < Server.s_globalDB[DatabaseID.SHEET_PAYMENT].length; i++)
			{
				...
				String product_id = Misc.PARSE_STRING(Server.s_globalDB[DatabaseID.SHEET_PAYMENT][i][DatabaseID.PAYMENT_PACK_ANDROID_PRODUCT_ID]);
...
//				LogHelper.Log("product_id := " + product_id);
		...
		res.put("TransactionID", _order_id);
